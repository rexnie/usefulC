...
int my_insert(struct rb_root *root, struct mytype *data)
{
	...
	while (*tmp) {
		...
		if (data->num < this->num)
			...
			tmp = &((*tmp)->rb_right);
		else /* return -1 if data->num existed */
			return -1;
	}

	/**
	 * if not found, parent point to parent of new node to be inserted,
	 * *tmp = new_node, tmp= &(parent->rb_left/right)
	 */
...
int main(int argc, char *argv[])
{
	...
	printf("\nthe second test, after deleting 21\n");
	print_rbtree(&mytree);
